...
export function DeleteConfirmModal({
    ...
    useEffect(() => {
        if (!isOpen || !hasAnchorTop) return;
        const timeout = window.setTimeout(() => {
            cardRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' });
        }, 80);
        return () => window.clearTimeout(timeout);
    }, [isOpen, hasAnchorTop, mobileAnchorTop]);
    ...
    const modalContent = (
        <div className="app-modal-overlay" style={{
            ...
            display: 'flex', alignItems: hasAnchorTop ? 'flex-start' : 'center', justifyContent: 'center', zIndex: 100,
            padding: '1rem', overflowY: 'auto'
        }}>
            <div ref={cardRef} style={{ width: '100%', maxWidth: '450px', marginTop: hasAnchorTop ? `${Math.max(0, mobileAnchorTop + 36)}px` : undefined }}>
