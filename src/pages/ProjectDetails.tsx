...
const DocumentTreeNode = ({ node, level = 0, activeCategoryId, onSelectCategory, activeFolder, setFolderStack }: any) => {
    ...
    return (
        <div style={{ userSelect: 'none' }}>
            ...
            >
                {isCategory ? (
                    node.id === 'cat-contactos' ? <Users size={16} style={{ color: isActive ? 'var(--color-primary)' : 'var(--text-muted)' }} /> :
...
export default function ProjectDetails() {
    ...
    // Dummy data for contacts and agents for EventReport
    const assignedContacts = [
        { id: 'c1', name: 'Juan Pérez', type: 'persona', role: 'Jefe de Obra' },
        { id: 'c2', name: 'Constructora XYZ', type: 'empresa', role: 'Contratista Principal' },
    ];
    ...
    const renderFileItem = (f: { id: string, name: string, path?: string, size: number, uploadDate: string }, isGlobal: boolean = false) => (
        ...
        <div className="animate-fade-in">
                                    ...
                                    <div style={{ display: 'grid', gridTemplateColumns: 'minmax(250px, 1fr) minmax(250px, 1fr)', gap: '1.5rem', alignItems: 'start' }}>
                                            ...
                                            <div className="card-body flex flex-col gap-2" style={{ padding: '1rem', maxHeight: '400px', overflowY: 'auto' }}>
                                                ...
                                                <div className="p-3 border border-gray-200 rounded-md bg-white shadow-sm flex items-center justify-between">
                                                    <div>
                                                        ...
                                                        <span className="text-gray-600">Empresa Promotora SA</span>
                                                    </div>
                                                </div>
                                                <div className="p-3 border border-gray-200 rounded-md bg-white shadow-sm flex items-center justify-between">
                                                    <div>
                                                        <strong>Contratista Principal: </strong>
                                                        <span className="text-gray-600">Constructora XYZ</span>
                                                    </div>
                                                </div>
                                                {/* Others mapping */}
                                                {assignedContacts.filter(c => c.type === 'empresa' && c.role !== 'Contratista Principal').map((c) => (
                                                    <div key={c.id} className="p-3 border border-gray-200 rounded-md bg-white shadow-sm flex items-center justify-between">
                                                        <div>
                                                            <strong>{c.role}: </strong>
                                                            <span className="text-gray-600">{c.name}</span>
                                                        </div>
                                                        <Button variant="ghost" size="sm" className="text-red-500 hover:bg-red-50" style={{ padding: '4px' }}><Trash2 size={16} /></Button>
                                                    </div>
                                                ))}

                                                <div className="mt-2 text-sm">
                                                    <label className="text-gray-500 mb-1 block">Asignar existente:</label>
                                                    <select className="input-field py-1" defaultValue="">
                                        ...
                                        <Card style={{ backgroundColor: 'var(--color-surface)' }}>
                                            ...
                                            <div className="card-body flex flex-col gap-2" style={{ padding: '1rem', maxHeight: '400px', overflowY: 'auto' }}>
                                                ...
                                                <div className="p-3 border border-gray-200 rounded-md bg-white shadow-sm flex items-center justify-between">
                                                    <div>
                                                        <div className="text-xs text-primary font-bold mb-1"><FileBadge size={14} className="inline mr-1" />Técnico Redactor</div>
                                                        <span className="text-gray-700 font-medium">Arquitecto Ejemplo</span>
                                                    </div>
                                                </div>
                                                <div className="p-3 border border-gray-200 rounded-md bg-white shadow-sm flex items-center justify-between">
                                                    <div>
                                                ...
                                                {assignedContacts.filter(c => c.type === 'persona' && !['Técnico Redactor', 'Coordinador SyS'].includes(c.role)).map((c) => (
                                                    <div key={c.id} className="p-3 border border-gray-200 rounded-md bg-white shadow-sm flex items-center justify-between group">
                                                        ...
                                                        <Button variant="ghost" size="sm" className="text-red-500 opacity-0 group-hover:opacity-100 transition-opacity"><Trash2 size={16} /></Button>
                                                    </div>
                                                ))}
            ...
            {isPersonaModalOpen && (
                <PersonaModal
                    ...
                    onSave={() => {
                        // Normally this would dispatch save via store to the real DB 
